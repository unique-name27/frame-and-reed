...
function getv(id) { return NATIVE[id] ? conv(+$(id).value, units, NATIVE[id]) : +$(id).value; } // in the engine's units
function setv(id, v) { if (!NATIVE[id]) { $(id).value = v; return; } $(id).value = String(+conv(+v, NATIVE[id], units).toFixed(6)); }
...
$('sample').addEventListener('click', () => { const P = params(); $('t-dist').value = units === 'mm' ? +(P.len * IN).toFixed(1) : P.len; openTracer(sampleHarpImage(P), traceDone); });
